  ...
  parseDateString(dateText) {
    ...
    try {
      ...
      if (dateTimeMatch) {
        ...
        // Registrar el éxito en la conversión
        window.logManager.step(window.logManager.phases.EXTRACTION, 'DATE_PARSE', 
          `Fecha parseada: "${dateText}" → ${new Date(date).toISOString()}`, 
          {originalText: dateText, timestamp: date.getTime()});
      ...
      if (!isNaN(timestamp)) {
        window.logManager.step(window.logManager.phases.EXTRACTION, 'DATE_PARSE', 
          `Fecha parseada (formato alternativo): "${dateText}" → ${new Date(timestamp).toISOString()}`, 
          {originalText: dateText, timestamp});
        return timestamp;
      }
      
      window.logManager.step(window.logManager.phases.EXTRACTION, 'DATE_PARSE_FAIL', 
        `No se pudo parsear la fecha: "${dateText}"`, 
        {originalText: dateText});
      return null;
    } catch (error) {
      window.logManager.step(window.logManager.phases.EXTRACTION, 'DATE_PARSE_ERROR', 
        `Error parseando fecha "${dateText}": ${error.message}`, 
        {originalText: dateText, error});
      return null;
    }
  }
