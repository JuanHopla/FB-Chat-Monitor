...
class HumanSimulator {
  ...
  async insertTextNaturally(inputField, text) {
    ...
    // Possibility to insert with a typo then correct
    if (this.config.enableTypos && Math.random() < this.config.typingErrors.probability && text.length > 10) {
      const typo = this.createTypoVersion(text);
      if (typo !== text) {
        // Insert version with typo
        inputField.innerText = typo;
        inputField.dispatchEvent(new Event('input', { bubbles: true }));
        
        // Wait briefly
        const correctionDelay = this.config.typingErrors.correctionDelay[0] + 
                               Math.random() * (this.config.typingErrors.correctionDelay[1] - this.config.typingErrors.correctionDelay[0]);
