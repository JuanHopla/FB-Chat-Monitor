...
function initialize() {
  ...
  if (CONFIG.AI.apiKey) {
    ...
    if (window.openAIManager) {
      ...
      if (window.logManager) {
        window.logManager.step('INITIALIZATION', 'ERROR', 'openAIManager not available for auto-initialization');
      } else {
  ...
  if (window.openaiManager) {
    ...
    if (window.logManager) {
      window.logManager.step('INITIALIZATION', 'WARNING', 'OpenAI Manager not available. Some functions may not be operational.');
    } else {
  ...
  try {
    ...
    if (window.logManager) {
      window.logManager.phase('INITIALIZATION', `Error in initialization: ${error.message}`, error);
    } else {
...
async function initializeServices() {
  ...
  if (CONFIG.audioTranscription.enabled) {
    ...
    if (!window.audioTranscriber) {
      if (window.logManager) {
        window.logManager.phase('INITIALIZATION', 'ERROR', 'Audio transcriber not loaded. Transcription services will be unavailable.');
      } else {
